...
更新：2026-09-22。**第一批已完成；用户已批准第二批正文内插图，正在实施。** 用户已批准本批范围；本批仅代码、验收、推送main与CI，不交付安装包、不公开发布。已发布版本仍为v0.5.0/code6，其标签和附件保持不变。

## 第二批当前目标

- 用户已确认图片→隐私；删除便签拖动排序需求。原始需求仅作背景，不额外授权其他功能。
- 已批准正文内插图、默认图片限制、仅图片便签及摘要、含图片ZIP备份，见docs/V0.6-IMAGES.md。
- 数据和文件管理里程碑已完成：56项JVM、构建/Lint、API35八项专项通过。证据work/v06-images/m1-build.log、m1-device.log。数据提交1565c1c的CI35676376546成功。备份恢复里程碑也已完成：57项JVM及构建/Lint、API35八项备份专项通过，见work/v06-images/m2-final-*。下一步图文编辑和专项验收。
- 第一批日志与成果直接复用；正式签名包升级、公开发布、鸿蒙6真机及长期提醒异常仍不在范围。图片完成后停止，隐私另行确认。
...
- 本批代码、验收、推送和CI无尾项。第二批按已批准图片范围继续，勿重跑第一批完整QA。
...
- 图片已进入第二批；隐私便签待另行确认；拖动排序已取消。农历、重复任务、多时段、提前提醒、单次例外不自动加入。
...
- 专用API35 ARM64模拟器work/avd/Daybook_API_35.avd，emulator-5554；本轮测试完成后已核对字号恢复1.0并关闭，数据保留（work/v06/m3-emulator-stop.log）。不卸载/清库。
