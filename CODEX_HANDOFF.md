...
**v0.5 里程碑一已实现并完成本地专项验证，正在整理提交与推送；完成状态以 Git 和开发日志最后记录为准。用户只授权本里程碑提交推送，不发布最终安装包、不创建 Release。** v0.4.0 已发布并收尾，复用成果，不重做发布。既有提醒异常继续暂缓。
...
- 当前待收尾：提交并推送 main，核对本提交 CI，更新最终状态。除此以外不自动启动下一里程碑。
