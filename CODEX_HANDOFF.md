...
更新：2026-09-23。**用户已明确授权发布v0.7.0，发布尚未执行。AI完整辅助版四阶段实现/本地专项/CI均完成，当前仍为0.7.0-dev/code8。启动发布阶段前五小时剩余17%，按现行五小时18%阈值保存交接，没有开启正式构建。此处不是因周额度停止。以下最新状态优先于历史记录。**

## 最新发布交接（优先读取）

- 当前目标：按用户“全部完成了就发布吧”授权发布v0.7.0，不再询问同一发布授权。实施基线main=14a318f，应用代码5512b59及CI35818542035成功；本轮开始工作区干净。
- 本轮已完成：低成本核对交接、Git、构建说明、CI工作流及旧发布脚本位置。现有69项JVM、分阶段专项和CI直接复用。没有修改业务代码/版本、没有构建或启动模拟器、没有创建标签或Release、没有上传附件。
- 暂未完成：正式版本号从0.7.0-dev改为0.7.0（code8）、发布说明与用法状态更新；正式原签名APK构建/核验；针对真实v0.6→v0.7的同签名覆盖升级与正式启动检查；提交推送/对应CI；新标签与Release、源码ZIP和SHA256清单上传及远程大小/哈希验证；最终交接。
- 重要约束：用户已经知道真实DeepSeek/真机效果未验证，并据此授权发布。说明中如实保留验证边界，不伪称线上模型已测，不索取Key或擅自付费调用。v0.6及更早标签/正文/附件保持不变；不重跑完整QA，不降级/卸载/清库。
- 额度：此时五小时窗口used83%，剩17%；不把此百分比沿用到恢复时，不因周额度判断停止，不自动兑换重置信用。当前先保存交接，之后仅必要文档提交。
- 下一步：用户说“继续发布/继续”后先核对实际Git及本交接，复用已有结果，从版本与发布说明准备接着执行。正式升级使用独立5556模拟器（现有v0.6，先核验实际安装包版本/哈希），复用旧流程但不能运行断言v0.5的旧seed。可在本机work/v07-release中准备新脚本与证据，work/v06-release/verify_upgrade.py、verify_published.py仅作参考，勿改旧证据。
- 最后本轮文档提交号以git log为准；已有产物outputs/v0.6均保持不变。
