...
更新：2026-09-13。当前已完成用户批准的 v0.4.0 实现和专项验收，正在完成交付提交/附件。恢复时只核对实际 Git、CI 与已有输出，不重复实现或完整 QA，不自动开始 backlog。

## 当前成果

- v0.4.0 / versionCode 4，原签名保留。A/B `8ec8e4c`：单行年月、跟手分页和短滑回弹、具体时间未定、筛选重置。C `cfaec74`：独立便签。D `fd36ad0`：自选背景和本地配色。三个阶段都已推送 main，CI 均成功；最终交付代码以 git log 和 v0.4.0 标签为准。
...
- A/B CI 34751483037、C CI 34751845966、D CI 34761242134 成功。最终交付 CI/发布结果在收尾提交补记。
...
- 原专用 Daybook_API_35：ANDROID_AVD_HOME=$PWD/work/avd，已关闭，保留 QA 数据及 v0.4.0 QA 包。新独立升级实例：ANDROID_AVD_HOME=$PWD/work/v04/upgrade-avd，同名 AVD 但独立空白数据目录，当前为正式 v0.4.0 包。不得卸载清空原实例以便重跑升级。
