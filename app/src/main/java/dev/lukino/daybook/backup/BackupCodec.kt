...
object BackupCodec {
    ...
    fun encode(entries: List<Entry>, exportedAt: Long = System.currentTimeMillis()): String {
        validate(BackupArchive(3, exportedAt, entries))
        return json.encodeToString(BackupArchive(3, exportedAt, entries)).also {
    ...
    fun decode(text: String): BackupArchive {
        ...
        require(root["formatVersion"]?.jsonPrimitive?.intOrNull in 1..3) { "不支持此备份版本" }
        val entries = root["entries"]?.jsonArray ?: error("备份缺少记录列表")
        require(entries.size <= MAX_ENTRIES) { "备份最多支持 10000 条记录" }
        if (root["formatVersion"]?.jsonPrimitive?.intOrNull in 2..3) entries.forEach {
            require(it.jsonObject.keys.containsAll(setOf("reminderAt", "reminderDeliveredFor"))) { "备份提醒字段不完整" }
        }
        if (root["formatVersion"]?.jsonPrimitive?.intOrNull == 3) entries.forEach {
    ...
    private fun validate(archive: BackupArchive) {
        require(archive.formatVersion in 1..3 && archive.exportedAt >= 0) { "备份信息无效" }
